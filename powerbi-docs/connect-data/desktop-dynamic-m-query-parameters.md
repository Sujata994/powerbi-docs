...
As a prerequisite for this feature, you must have a valid [M Query Parameter](/power-query/power-query-query-parameters) created and referred in one or more Direct Query tables. 
 > [!NOTE]
    > Be sure to check the **Considerations and limitations** section as not all Direct Query sources are supported with this feature.
...
* The feature is only supported for M based data sources. The following Direct Query sources are not supported:
